...
$error = false;
$error = (!$error && (empty($title) || strlen($title) < 50)) ? "Title should not be blank or less than 50 characters" : $error;
$error = (!$error && strlen($title) > 250) ? "Title should not be more than 250 characters" : $error;
$error = (!$error && (empty($slug) || strlen($slug) < 20)) ? "Slug should not be blank or less than 20 characters" : $error;
$error = (!$error && strlen($slug) > 250) ? "Slug should not be more than 250 characters" : $error;
$error = (!$error && (empty($description) || strlen($description) < 100)) ? "Description should not be blank or less than 120 characters" : $error;
